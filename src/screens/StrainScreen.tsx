...
import { getAllStrainImages, hasMultipleImages, getStrainImage, getStrainImageSync } from '../utils/imageUtils';
...
const strainImages: Record<string, string> = {
  ...
  'strain-4': 'northern_lights_1.png',
  'strain-5': 'jack_herer_1.png',
  'strain-6': 'pineapple_express_1.png',
  'strain-7': 'gsc_1.png',
};
...
export const StrainScreen = ({ route, navigation }: Props) => {
    ...
    try {
      ...
      if (supabaseStrain) {
        ...
        if (foundStrain) {
          ...
          reviewCache.set(foundStrain.id, reviews);
        } else {
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      // Update the reviewCache with the fetched reviews
      if (strain) {
        reviewCache.set(strain.id, data || []);
      } else {
        console.warn(`Strain not found with ID: ${strainId}`);
      }
    } catch (error) {
  ...
  const getStrainImages = useCallback((): ImageSourcePropType[] => {
    console.log('Getting strain images in StrainScreen');
    
    // Early return if strain is undefined
    if (!strain) {
      console.log('No strain data available, using default image');
      return [DEFAULT_STRAIN_IMAGE];
    }
    
    // Create a local variable to avoid TypeScript errors
    const strainData = strain;
    console.log(`Strain image_url: ${strainData.image_url || 'undefined'}`);
    
    try {
      // First try to load local images directly using require
      // This is the most reliable way to load images in React Native
      if (strainId === 's1') {
        console.log('Loading s1 (Blue Dream) image from local assets');
        return [require('../../assets/images/strains/s1/blue_dream_1.jpg')];
      } else if (strainId === 's2') {
        console.log('Loading s2 image from local assets');
        try {
          return [require('../../assets/images/strains/s2/og_kush_1.jpg')];
        } catch (e) {
          console.log('Error loading s2 image:', e);
        }
      } else if (strainId === 's3') {
        console.log('Loading s3 image from local assets');
        try {
          return [require('../../assets/images/strains/s3/sour_diesel_1.jpg')];
        } catch (e) {
          console.log('Error loading s3 image:', e);
        }
      }
      
      // Use the synchronous version for all other cases
      return [getStrainImageSync(strain)];
    } catch (error) {
      console.error('Error in getStrainImages:', error);
      return [DEFAULT_STRAIN_IMAGE];
    }
  }, [strain, strainId]);
  ...
  // Update the strainImages variable to use our getStrainImages function
  const strainImagesArray = useMemo(() => {
    return getStrainImages();
  }, [getStrainImages]);
        ...
        <View style={styles.container}>
          <ScrollView>
            <View style={styles.heroSection}>
              <Image 
                source={strainImagesArray[0] || DEFAULT_STRAIN_IMAGE} 
  ...
  const renderHistogramBar = (rating: number, count: number) => {
    // If there are no reviews, show empty bars with 0% width
    const percentage = reviewStats.totalReviews > 0 
      ? (count / reviewStats.maxCount) * 100
      : 0;
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        ...
        {/* Image gallery if multiple images exist - hidden since we only support single images now */}
        {false && strainImagesArray.length > 1 && (
          <View style={styles.imageGallery}>
            <FlatList
              horizontal
              data={strainImagesArray}
              ...
              renderItem={({ item, index }) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    index === activeImageIndex ? styles.activeGalleryImage : {}
                  ]}
                >
                  <Image 
                    source={item} 
        ...
        <View style={styles.content}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.tags}>
              {strain.effects && strain.effects.length > 0 ? (
                ...
                <Text style={styles.noDataText}>No effects data available</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.tags}>
              {strain.flavors && strain.flavors.length > 0 ? (
                ...
                <Text style={styles.noDataText}>No flavor data available</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            {strainReviews && strainReviews.length > 0 ? (
                ...
                >
                  <View style={styles.reviewHeader}>
                    ...
                    >
                      ...
                      />
                      <Text style={styles.username}>
                        {review.profiles?.username || mockUsers.find(user => user.id === review.user_id)?.username || 'Anonymous'}
                      </Text>
                    </TouchableOpacity>
